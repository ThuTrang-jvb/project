...
const CountryPage = (): React.ReactElement => {
  ...
  // Overlay loading hiển thị mượt, không chớp
  useEffect(() => {
    if (moviesLoading) {
  ...
  // Cập nhật phim sau khi loading xong
  useEffect(() => {
    if (!moviesLoading) {
  ...
  // Đánh dấu đã từng có dữ liệu phim
  useEffect(() => {
    if (movies.length > 0 && !hasLoadedOnce) {
      setHasLoadedOnce(true)
    }
  }, [movies, hasLoadedOnce])
  ...
  return (
    <div className="genre-movie-list" style={{ position: "relative", minHeight: "80vh" }}>
      ...
      {/* Hiện Loading... chỉ nếu chưa có phim lần nào */}
      {movies.length === 0 && isOverlayVisible && !hasLoadedOnce && (
        <p>Loading...</p>
      )}

      {/* Hiện No result nếu không có phim nào sau khi load */}
      {movies.length === 0 && !isOverlayVisible && hasLoadedOnce && (
        <p>No results found.</p>
      )}

      {/* Hiển thị phim kèm overlay nếu đang tải */}
      {movies.length > 0 && (
        <>
          {isOverlayVisible && (
